...
ggplot(ranked, aes(x=date, y=rank)) +
  geom_point(aes(color = bird_breed), shape=17) +
  geom_line(aes(color = bird_breed)) +
  geom_text(data = subset(ranked, ranked$date == min(ranked$date)), 
            aes(date, rank, label = bird_breed, col=bird_breed), size = 2,
...
# Save
ggsave('expected_plots/script3_correction.png', width=10, height=8)
